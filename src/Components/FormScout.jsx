...
export const FormScout = ({
  time,
  desfazerPonto,
  quarter = 1,
  encerrarQuarter,
}) => {
  const { scoutA, registrarCesta } = useContext(TimeA_Context);
  const { scoutB, registrarCestaB } = useContext(TimeB_Context);
  ...
  function handleSubmitScout(e) {
    ...
    if (time === "timeA") {
      // registrarCesta agora cria o jogador caso não exista, então
      // não precisamos chamar adicionarJogador antes.
  ...
  return (
    <form
      onSubmit={handleSubmitScout}
      className="space-y-2 flex flex-col bg-white p-4 rounded shadow w-full max-w-xs mx-auto md:max-w-md"
    >
      <div className="flex items-center gap-2 w-full">
        <label
          htmlFor="numero_jogador"
          className="text-sm md:text-base mb-0 flex-1"
        >
          Digite o número do jogador
        </label>
        {desfazerPonto &&
          (time === "timeA" ? scoutA.length > 0 : scoutB.length > 0) && (
            <button
              type="button"
      ...
      <input
        ...
        className={`border-2 ${
          jogadorPisca
            ? "border-red-500 animate-[blink_0.3s_2]"
            : "border-gray-300"
        } rounded px-2 py-2 focus:outline-none text-base md:text-lg`}
        inputMode="numeric"
        pattern="[0-9]*"
        ...
        spellCheck={false}
      />
      <label htmlFor="valor_cesta" className="text-sm md:text-base">
        Valor da Cesta?
      </label>
      <input
        type="tel"
